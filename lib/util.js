...
function getLogger(options){
    if(_.isString(options)) {
        loggerOptions.name = options;
    }
    else if(_.isObject(options)) {
        loggerOptions = _.merge(loggerOptions, options);
    }

    if(!logger) {
        logger = stumpy(loggerOptions);
        return logger;
    } else {
        // TODO: better way to copy options from logger to logger
        var opt = logger.getOptions();
        loggerOptions.env = opt.env;

        return logger.shadow(loggerOptions);
    }
}
