...
Hyper.prototype._normalizeServicesConfig = function(servicesConfig) {
    ...
    if (_.isArray(servicesConfig)) {
        ...
        for (var i = 0; i < tmp.length; i++) {
            ...
            if (!fs.existsSync(servicesConfig.services[appName].directory)) {
                ...
                logger.info("Could not find service directory:", servicesConfig.services[appName].directory);
            }
    ...
    if (!servicesConfig.hasOwnProperty('services')) {
        ...
        if (servicesConfig.hasOwnProperty('preRoutes')) {
            // move preRoutes to in service object
            servicesConfig.services[defaultAppName].preRoutes = servicesConfig.preRoutes;
            delete servicesConfig.preRoutes;
        }
