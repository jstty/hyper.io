...
// set/get server port
HttpFrameworkExpress.prototype.protocol = function () {
  return 'http' + (this._options.credentials ? 's' : '') + ':';
};

// set/get server port
HttpFrameworkExpress.prototype.port = function (nPort) {
  if (nPort) {
...
// load all services
HttpFrameworkExpress.prototype.load = function () {
...
HttpFrameworkExpress.prototype.validateMethod = function (method) {
  if (!this._app[ method ] ||
...
HttpFrameworkExpress.prototype.getName = function () {
  return 'express';
};

HttpFrameworkExpress.prototype.app = function () {
  return this._app;
};

/*
HttpFrameworkExpress.prototype.server = function() {
    return this._server;
};
*/

HttpFrameworkExpress.prototype.addMethodFunction = function (method, middlewareList, routeStr, handler) {
...
HttpFrameworkExpress.prototype.addStaticDir = function (staticDir, staticRoute, staticOptions, staticDefaultFile) {
  if (staticRoute) {
    return this._app.use(staticRoute, function (req, res, orgNext) {
      var next = orgNext;

      if(staticDefaultFile) {
        staticOptions = staticOptions || {};
        staticOptions.fallthrough = false; // to allow to catch error and handle it
        ...
        next = function (err) {
          ...
          if(err.code === 'ENOENT') {
            // use default
            fs.readFile(staticDefaultFile, function(err, content) {
              if (err) {
                logger.error('Missing Default Static File - Error:', err);
              }
              else {
                var mimetype = mime.getType(staticDefaultFile);
                res.writeHead(200, { 'Content-Type': mimetype });
                res.end(content, 'utf-8');
              }
            });
          } else {
            logger.error('Default Static Dir - Error:', err);
          }
        }
      }

      var staticFunc = express.static(staticDir, staticOptions);
...
HttpFrameworkExpress.prototype.addStaticFile = function (staticRoute, staticFile) {
  this._app.get(staticRoute, function (req, res) {
    res.sendFile(path.resolve(staticFile));
  });
};

HttpFrameworkExpress.prototype.addStaticFileDefault = function (staticFile) {
  this._app.use(function (req, res) {
    res.sendFile(path.resolve(staticFile));
  });
};

HttpFrameworkExpress.prototype.addRedirect = function (from, to) {
...
HttpFrameworkExpress.prototype._getExpressLogger = function () {
...
HttpFrameworkExpress.prototype.buildInputs = function ($rawRequest) {
  // POST - req.body
  // GET  - req.query
  // GET  - req.params
...
// templateFunc
HttpFrameworkExpress.prototype.addWrappedMethodFunction = function (method, middlewareList, routeStr, handler) {
  this.addMethodFunction(method, middlewareList, routeStr, function (req, res, next) {
    var responded = false;

    handler(this.buildInputs(req), req.session, req.cookies, req, res, next)
      .then(function (output) {
        ...
        if (output.headers.filename) {
          var mimetype = mime.getType(output.headers.filename);
 ...
 * @private
 */
HttpFrameworkExpress.prototype.validateInputs = function (cInput, req) {
  var errors = [];
