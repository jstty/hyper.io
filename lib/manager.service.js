        ...
        .then(function(){
            ...
            _.forEach(this._servicesConfig, function(service, key) {
                ...
                for(var d in this._services[key].directory) {
                    this._services[key].directory[d] = this._findDir(d, service.directory[d], key);
...
ServiceManager.prototype._findDir = function(type, directory, serviceName) {
    ...
    if(type === "service") {
        ...
        if (globs.length > 0) {
            ...
            return process.cwd() + path.sep + globs[0];
        }
        ...
        var globs = glob.sync('**' + path.sep + type);
        // check if file exists
        if (globs.length > 0) {
            if (globs.length > 1) {
                logger.warn("More than one service file found", globs);
        ...
        // default "<cwd>/<type>"
        return process.cwd() + path.sep + type;
    }
};
