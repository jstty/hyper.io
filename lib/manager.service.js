...
//
var ServiceManagerConfig = require('./manager.service.config.js');

var ResourceHandler   = require('./handler.resource.js');
var DIManager         = require('./manager.service.di.js');

var PipelineManager   = require('./manager.pipeline.js');
 ...
 * Constructor
 * --------------------------------------------------- */
function ServiceManager(hyperCore, middleware, serviceRouter, defaultAppName, config, servicesConfig) {
    ...
    var config = new ServiceManagerConfig(defaultAppName, servicesConfig); // normilizes the configs
    this._servicesConfig = config.get();

    this._services  = {};

    this._middleware = middleware;
    this._pipeline = new PipelineManager();
...
ServiceManager.prototype.loadHttpFramework = function() {
    ...
    //.catch(function(err){
    //    logger.error('Error Loading HTTP Framework:', err);
    //}.bind(this));
};
...
ServiceManager.prototype.loadServices = function() {
    ...
    this._pipeline.add(require('./pipeline.validate.js'));
    this._pipeline.add(require('./pipeline.resolver.js'));
    this._pipeline.add(require('./pipeline.controller.js'));
    this._pipeline.add(require('./pipeline.template.js'));

    return this._loadServices();
    //.catch(function(err){
    //    logger.trace('Error Loading Services:', err);
    //}.bind(this));
...
// ------------------------------------------------
        ...
        _.forEach(this._servicesConfig.services, function(serviceConfig, key) {
            this._services[key] = {};
            var service         = this._services[key];
            service.name        = key;
            service.config      = serviceConfig.config  || {};
            service.options     = serviceConfig.options || {};
            service.routes      = serviceConfig.routes  || {};
            service.preRoutes   = serviceConfig.preRoutes;
            service.controller  = {};
            service.resolver    = {};
            service.directory   = { service: "", controllers: "", resolvers: "", views: "", static: ""};
            service.resources   = {};
            ...
            //if(!serviceConfig.directory) { serviceConfig.directory = service.directory; }
            /*
            if(_.isString(serviceConfig.directory)) {
                var dir = serviceConfig.directory;
                serviceConfig.directory = { service: dir, controllers: "", resolvers: "", views: "", static: ""};
            }
            */

            /*
            // find dir for each type
            for(var d in service.directory) {
                service.directory[d] = this._findDir(d, serviceConfig.directory[d], key, service.directory.service);
            ...
            */
            ...
            this._loadDefaultServiceMiddleware(service);

            // module default: "./<directory>/<service name>.js"
            if(!serviceConfig.module) {
                service.module = this._loadServiceFile(key, service.directory.service);
            ...
            if( service.module ) {
                ...
                var dim = new DIManager(this._hyperCore, this._serviceRouter);
                dim.add(module);
                dim.add(this._resources); // shared resources
                dim.add(service.resources); // just service resource
                dim.setParent(service);
                service.instance = dim.inject();
        ...
        _.forEach(this._services, function(service, key) {
            // create instance of module
            if( service.module &&
                service.preRoutes &&
                _.isFunction(service.preRoutes) ) {

                logger.group("Loading Service "+key+" - PreRoutes...");
        ...
        _.forEach(this._services, function(service, key) {
            // create instance of module
            if( service.module ) {
                logger.group("Loading Service "+key+" - Setup Routes...");
 ...
 */
// TODO: move all resources stuff to it's own class
ServiceManager.prototype.addResource = function(name, resourceModule, type, service) {
    ...
    if(type == 'factory') {
        if (_.isFunction(resourceModule)) {
            ...
            var dim = new DIManager(this._hyperCore, this._serviceRouter);
            dim.setParent( { module: resourceModule } );
            dim.add(this._resources); // shared resources
            dim.add(service.resources); // just service resource

            resourceModule = dim.inject();
        } else {
...
ServiceManager.prototype._setupRoutes = function(service) {
    _.forEach(service.routes, function(route) {
        ...
        if( route.hasOwnProperty('api') ) {
            // default
            if(!route.pipeline) {
                route.pipeline = ['validate', 'resolver', 'controller'];
            }
            ...
            // default
            if(!route.pipeline) {
                route.pipeline = ['validate', 'resolver', 'controller', 'template'];
            }
...
// TODO: move controller to it's own class
    ...
    if(!service.controller[controllerName].instance) {
        if(_.isFunction(service.controller[controllerName].module)) {
            var module = {
                ...
                '$options':  service.options[ controllerName ],
                '$logger':   util.logger(service.name + ' - ' + controllerName) // TODO: add logger to controller object
            };

            var dim = new DIManager(this._hyperCore, this._serviceRouter);
            dim.setParent( service.controller[controllerName] );
            dim.add(module);
            dim.add(this._resources); // shared resources
            dim.add(services.resources); // just service resource
            service.controller[controllerName].instance = dim.inject();
        } else {
...
// TODO: remove this in favor of the DIManager
...
ServiceManager.prototype.setupDynamicRoute = function(type, service, controller, route) {
    ...
    _.forEach(route.method, function(value, m) {
        ...
        this._httpFramework.addMethodFunction(m, middlewareList, routeStr,
            // TODO: Make this geneic, support with both express and koa
            ...
            this._pipeline.run(route, module);
            /*
            // run pipeline
            var tasks = [];
            _.forEach(route.pipeline, function(pipeline) {
                var t = null;
                // ---------------------------------------
