...
//
var ServiceManagerConfig = require('./manager.service.config.js'); // ES6
...
function ServiceManager(hyperCore, appConfig, servicesManifest, middleware, httpFramework, defaultAppName) {
    ...
    // used to wait resourced promise Queue
...
// ------------------------------------------------
        ...
        this._resourceQueue = [];
        this._resourceQueue.push(when.resolve());

        _.forEach(this._servicesManifest.services, function(serviceManifest, key) {
            this._services[key] = {};
            ...
            // set dir to serviceManifest
            //if(!serviceManifest.directory) { serviceManifest.directory = service.directory; }
            /*
            if(_.isString(serviceManifest.directory)) {
                var dir = serviceManifest.directory;
             serviceManifest.directory = { service: dir, controllers: "", resolvers: "", views: "", static: ""};
            }
            */
            ...
            for(var d in service.directory) {
                service.directory[d] = this._findDir(d, serviceManifest.directory[d], key, service.directory.service);
        ...
        return when.all(this._resourceQueue)
            .then(function(){
        ...
        if (_.isFunction(resourceModule)) {
            ...
            if(_.isFunction(resourceModule.$init)) {
                try {
                    ...
                    // is promise
                    if(_.isObject(result) && _.isFunction(this._resourceQueue.then)) {
                        this._resourceQueue.push(result);
                    }
                    else if(result) {
                        this._resourceQueue.push( when.resolve(result) );
                    }
                }
                catch(err) {
